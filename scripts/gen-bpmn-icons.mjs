...
// Apache-2.0) source in the @iconify-json/mdi devDependency. Run: `node scripts/gen-bpmn-icons.mjs`.
//
...
const MAP = [
  ...
  { name: 'compensation-in', from: 'rewind-outline' },
  { name: 'compensation-out', from: 'rewind' },
];
...
const CROSS = '9.5,2 14.5,2 14.5,9.5 22,9.5 22,14.5 14.5,14.5 14.5,22 9.5,22 9.5,14.5 2,14.5 2,9.5 9.5,9.5';
...
// is the shape's opening tag up to (but not including) fill/stroke.
function markerPair(name, open) {
  return {
    [`${name}-in`]: {
      body: `${open} fill="none" stroke="currentColor" stroke-width="1.25" stroke-linejoin="round"/>`,
    },
    [`${name}-out`]: { body: `${open} fill="currentColor"/>` },
  };
}

const LITERAL = {
  // exclusive: an "X" (kept 2px smaller than the others so its thick strokes clear)
  exclusive: {
  ...
  ...markerPair('signal', `<polygon points="${TRI}"`),
  ...markerPair('escalation', `<polygon points="${ARROWHEAD}"`),
  ...
  'termination-out': { body: '<circle cx="12" cy="12" r="10" fill="currentColor"/>' },
...
const icons = {};
for (const { name, from, rotate, hFlip, vFlip } of MAP) {
  ...
  // requested flip toggles (XOR) any flip the source already carries.
...
const header = `// GENERATED by scripts/gen-bpmn-icons.mjs — do not edit by hand.
//
